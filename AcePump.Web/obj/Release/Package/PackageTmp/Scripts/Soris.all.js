...
//Soris.Ajax.InlineEditor class
(function (Soris, $, undefined) {
    "use strict";
   
    var Ajax = Soris.namespace("Ajax");
    
    var InlineEditor = Soris.Ui.UiComponentBase.extend(function (options) {
        ...
        this._EntityTypeName = options.EntityTypeName;
    ...
    InlineEditor.prototype.Render = function (originalDOMElement) {
        var orig = $(originalDOMElement);
        this._Value = this._PreviousValue = orig.val();

        var container = $(document.createElement("div"));
        container.addClass("s-inline-editor-container");
        container.prop("id", orig.prop("id"));

        this._RenderDisplay(container);
        this._RenderEditor(container);
        this._RenderErrorDisplay(container);
        this._ShowDisplay();

        container.data("sorisInlineEditor", this);

        orig.replaceWith(container);
    };

    InlineEditor.prototype._RenderDisplay = function(container) {
        this._Display = $("<span>");
        this._Display.addClass("s-display");
        
        var e = this;
        this._Display.on("click", function() {
            e._ShowEditor();
        });
    ...
    InlineEditor.prototype._RenderEditor = function(container) {
        this._Editor = $("<input>");
        this._Editor.addClass("s-editor");
        this._Editor.val(this._Value);

        this.UiComponentFactory.RenderEditor(this._Editor, {});
        ...
        container.append(this._Editor);
    };

    InlineEditor.prototype._RenderErrorDisplay = function(container) {
        this._ErrorDisplay = $("<span>");
        this._ErrorDisplay.addClass("s-error-display");
        
        container.append(this._ErrorDisplay);
    };

    InlineEditor.prototype._ShowEditor = function() {
        this._Display.hide();
        this._Editor.show();
        this._Editor.focus();
    };
    
    InlineEditor.prototype._ShowDisplay = function() {
        if (this._Value === "") {
            this._Display.text(this._PropertyName);
        } else {
            this._Display.text(this._Value);
        }
    ...
    InlineEditor.prototype._HandleFailedUpdate = function(errors) {
        this._Value = this._PreviousValue;
        this._ShowEditor();

        var fullErrorString = "";
        for (var i = 0; i < errors.length; i++) {
            fullErrorString += errors[i] + "<br />";
        }

        this._ErrorDisplay.html(fullErrorString);
    };
    
    InlineEditor.prototype._Update = function() {
        this._ErrorDisplay.text("");
        this._Value = this._Editor.val();
        var editor = this;
        ...
        $.ajax({
            ...
            success: function(data, xhr) {
                if (data.Result !== "Success") {
                    editor._HandleFailedUpdate(data.Errors);
                }
            },
            error: function() {
                editor._HandleFailedUpdate(["An error occured while saving your change, please try again."]);
            }
        });
    };
    
    Ajax.InlineEditor = InlineEditor;
})(window.Soris, jQuery);
//jQuery extensions
(function (Soris, $, undefined) {
    "use strict";

    $.fn.sorisInlineEditor = function (options) {
        this.each(function() {
            var editor = new Soris.Ajax.InlineEditor(options);
